...
import { CoreCategory, CoreCategoryKey } from "./typs";
...
export default function EventSidebar({
  ...
  return (
    <div className="h-full rounded-2xl border border-white/10 bg-neutral-950/70 p-4 backdrop-blur-xl">
      {/* Mobile header */}
      {isMobile ? (
      ...
      <div className="mb-6">
        ...
        <nav className="space-y-1">
          ...
          />
          <SidebarLink
            label="Go Together Board"
            active={activeNav === "TOGETHER"}
            onClick={() => onNavChange("TOGETHER")}
          />
        </nav>
      </div>
      ...
      <div>
        ...
        <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-white/70">
          ...
          <div className="mt-3 text-xs text-white/50">
            Next step: connect to Django API: <span className="text-white/70">/api/society/events</span>
          </div>
        </div>
      </div>
    </div>
